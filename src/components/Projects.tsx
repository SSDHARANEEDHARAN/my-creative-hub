...
/* ------------------------------------------------------------------ */
/*  Card frame helper — machined engineering panel look               */
/* ------------------------------------------------------------------ */
const CornerBrackets = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  const s = size === "sm" ? "w-3 h-3" : size === "lg" ? "w-6 h-6" : "w-5 h-5";
  return (
    <>
      <div className={`absolute top-0 left-0 ${s} border-t border-l border-primary/20 group-hover/card:border-primary/60 transition-colors duration-500 pointer-events-none`} />
      <div className={`absolute top-0 right-0 ${s} border-t border-r border-primary/20 group-hover/card:border-primary/60 transition-colors duration-500 pointer-events-none`} />
      <div className={`absolute bottom-0 left-0 ${s} border-b border-l border-primary/20 group-hover/card:border-primary/60 transition-colors duration-500 pointer-events-none`} />
      <div className={`absolute bottom-0 right-0 ${s} border-b border-r border-primary/20 group-hover/card:border-primary/60 transition-colors duration-500 pointer-events-none`} />
    </>
  );
};

/* ------------------------------------------------------------------ */
/*  Memoized link blocks                                              */
/* ------------------------------------------------------------------ */
...
const Projects = () => {
  ...
  const {
    viewCounts: industrialViewCounts,
    likeCounts: industrialLikeCounts,
    readCounts: industrialReadCounts,
    commentCounts: industrialCommentCounts,
  } = useProjectListCounts(industrialIds);
    ...
    <section id="projects" className="py-16 sm:py-20 md:py-24 relative overflow-hidden bg-mesh">
      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <motion.div
          ...
          className="grid sm:grid-cols-2 gap-5 sm:gap-7 mb-10"
        >
          {featuredProjects.map((project, index) => (
            <motion.div key={project.id} variants={featuredCardVariants} custom={index}>
              <div className="group/card relative bg-card border border-border overflow-hidden hover:border-primary/50 transition-all duration-500 hover-lift">
                <CornerBrackets size="lg" />
                {/* Left accent bar */}
                <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-primary/10 group-hover/card:bg-primary/40 transition-colors duration-500" />

                {/* Image */}
                <div className="relative overflow-hidden aspect-[16/10]">
                  {project.images && project.images.length > 0 ? (
                    <>
                      <div className="transition-transform duration-700 ease-out group-hover/card:scale-105">
                  ...
                  <div className="absolute top-4 left-4 z-20">
                    <span
                      className={`px-3 py-1 text-xs font-medium ${
                        activeTab === "it" ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"
                      }`}
                    >
                      Featured
                    </span>
                  </div>
                  <div className="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover/card:opacity-100 transition-all duration-300 translate-y-2 group-hover/card:translate-y-0 z-20">
                ...
                <div className="p-6 relative">
                  <h3 className="font-display text-xl font-semibold mb-2 group-hover/card:text-primary transition-colors flex items-center gap-2">
                    {project.title}
                    <ArrowUpRight size={18} className="opacity-0 group-hover/card:opacity-100 transition-opacity" />
                  </h3>
        ...
        <motion.div
          ...
          className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5 mb-10"
        >
          {otherProjects.map((project) => (
            <motion.div key={project.id} variants={smallCardVariants}>
              <div className="group/card relative bg-card border border-border overflow-hidden hover:border-primary/40 transition-all duration-500 hover-lift p-4">
                <CornerBrackets size="sm" />
                {/* Top accent bar */}
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-primary/10 group-hover/card:bg-primary/40 transition-colors duration-500" />
                ...
                <h3 className="font-display text-base font-semibold mb-2 group-hover/card:text-primary transition-colors">
                  {project.title}
                </h3>
            ...
            {!user ? (
              ...
              >
                {industrialProjects.map((project) => {
                  ...
                  return (
                    <motion.div key={project.id} variants={industrialCardVariants}>
                      <div className="group/card relative bg-card border border-border overflow-hidden hover:border-primary/50 transition-all duration-500 hover-lift">
        ...
        <div className="text-center">
          <Button variant="outline" size="lg" className="border-primary text-primary hover:bg-primary hover:text-primary-foreground border-2">
            View All Projects
          </Button>
        </div>
      </div>
